...
if not env_files or growth_file is None:
    ...
    with tab1:
        ...
        star = alt.Chart(ln_df[ln_df["is_max"]]).mark_point(shape="star", size=220, color="#ef4444").encode(
            x=alt.X("EC(설정):O", sort=[1,2,4,8], axis=axis_x),
            y=alt.Y(f"{ycol}:Q", axis=axis_y)
        )
        ...
        ).properties(title="차트 2 · 학교별 TOP 4", height=330)

        with g2:
            st.altair_chart(bar + text, use_container_width=True)

        # (섹션 3) 차트 3: 3지표 정규화 (0-100)
    ...
    with tab2:
        ...
        env_map = {"온도":"평균 온도","습도":"평균 슥도","EC":"평균 EC(측정)","pH":"평균 pH"}
        # 오타 방지: 직접 컬럼 지정
